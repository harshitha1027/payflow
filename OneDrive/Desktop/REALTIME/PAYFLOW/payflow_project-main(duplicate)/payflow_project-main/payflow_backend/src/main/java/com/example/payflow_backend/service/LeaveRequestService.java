...
public class LeaveRequestService {
    ...
    public double calculateDeductions(Long empId, int month, int year) {
        List<LeaveRequest> leaves = leaveRepo.findByEmployeeEmployeeId(empId);
        long unpaidDays = leaves.stream()
                .filter(leave -> leave.getStatus() == LeaveStatus.APPROVED &&
                        leave.getStartDate().getMonthValue() == month &&
                        leave.getStartDate().getYear() == year 
                       ) // optional check
                .mapToLong(leave -> ChronoUnit.DAYS.between(leave.getStartDate(), leave.getEndDate()) + 1)
                .sum();

        // Example: deduction = 100 per unpaid day
        return unpaidDays * 100.0;
    }
